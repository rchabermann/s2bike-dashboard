...
export function SpendChart({ data }: { data: DailyMetrics[] }) {
    ...
    <div className="card p-6 animate-in stagger-5">
      ...
      <ResponsiveContainer width="100%" height={260}>
        <ComposedChart data={formatted} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
          ...
          <Bar yAxisId="left" dataKey="impressions" name="Impressões" fill="#4f8ef7" opacity={0.85} radius={[3,3,0,0]} maxBarSize={28} />
          <Line yAxisId="right" type="monotone" dataKey="linkClicks" name="Cliques" stroke="#f97316" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  );
